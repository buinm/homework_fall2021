...
class DQNAgent(object):
    ...
    def step_env(self):
        ...
        # Time-varying exploration rate
        eps = self.exploration.value(self.t)

        # TODO use epsilon greedy exploration when selecting action
        # random number (0, 1), check if it's greater then eps then take the action if so
        rdn_num = np.random.random()
        perform_random_action = rdn_num < eps or self.t < self.learning_starts
        ...
        self.last_obs, rew, done, _ = self.env.step(action)
